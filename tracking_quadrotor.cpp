...
Eigen::Vector4d PX4Tracking::TrackingPidProcess(Eigen::Vector3d &currentPos,Eigen::Vector3d &expectPos,Eigen::Vector4d &orientation)
{
 // Eigen::Vector3d s_PidOut;
    ...
    s_PidOut[0] =xpid[0]*xerro+xpid[2]*xsumE;
	//+xpid[2]*xerro_c*100 
  ...
  /*s_PidOut[2]为飞机坐标系下Z轴也就是飞机上下方向的速度*/
	s_PidOut[2] = 1*(s_PidZ.p*s_PidItemZ.difference + s_PidZ.d*s_PidItemZ.differential + 0.05*s_PidItemZ.intergral);
	//  cout<<"上下速度："<<s_PidOut[2]<<endl;
	printf("前后速度:%f, 左右速度: %f, 上下速度:%f\n",s_PidOut[0], s_PidOut[3], s_PidOut[2] );
